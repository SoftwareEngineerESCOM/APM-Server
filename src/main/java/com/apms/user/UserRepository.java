...
public interface UserRepository extends JpaRepository<User, Integer> {
	@Query(value = "SELECT * FROM user_apms WHERE human_resource_id IN (SELECT id FROM human_resource WHERE id IN (SELECT hrp.human_resource_id FROM human_resource_positions hrp, human_resource_workplaces hrw WHERE hrp.positions_id = :position_id AND hrw.workplaces_id = :workplace_id AND hrp.human_resource_id = hrw.human_resource_id))", nativeQuery = true)
	List<User> getUsersByWorkplaceIdAndPositionId(@Param("workplace_id") Integer workplace_id, @Param("position_id") Integer position_id);
	...
	@Query(value = "SELECT * FROM user_apms WHERE human_resource_id IN (SELECT human_resource_id FROM human_resource_workplaces WHERE workplaces_id = :id)", nativeQuery = true)
	...
	@Query(value = "SELECT u.* FROM user_apms u, user_apms_roles ur, human_resource hr WHERE (u.id = ur.user_apms_id AND ur.roles_id IN (SELECT id FROM role WHERE rank < (SELECT MAX(rank) FROM role WHERE id IN (SELECT roles_id FROM user_apms_roles WHERE user_apms_id = :id )) AND rank > (SELECT MAX(rank) FROM role WHERE id IN (SELECT roles_id FROM user_apms_roles WHERE user_apms_id = :id )) - 3)) AND (u.human_resource_id IN (SELECT human_resource_id FROM human_resource_workplaces WHERE workplaces_id = :workplace_id)) GROUP BY u.id", nativeQuery = true)
	...
	@Query(value = "SELECT u.* FROM user_apms u, user_apms_roles ur WHERE (u.id = ur.user_apms_id AND ur.roles_id = :role_id) AND (u.human_resource_id IN (SELECT human_resource_id FROM human_resource_workplaces WHERE workplaces_id = :workplace_id)) GROUP BY u.id", nativeQuery = true)

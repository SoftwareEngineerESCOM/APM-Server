...
public interface LearningUnitTaskRepository extends JpaRepository<LearningUnitTask, Integer> {
    ...
    @Query(value = "SELECT * FROM learning_unit_task where learning_unit_id = :idLU AND user_id = :id", nativeQuery = true)
    List<LearningUnitTask> existUserLearningUnitByUserId(@Param("id") Integer id, @Param("idLU") Integer idLU);

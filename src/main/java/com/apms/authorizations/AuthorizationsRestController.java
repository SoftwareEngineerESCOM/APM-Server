...
public class AuthorizationsRestController {
    ...
    @GetMapping("getAuthorizationsByLearningUnitId/{id}")
    public RESTResponse<Authorizations> getAuthorizationsByLearningUnitId(@PathVariable Integer id) {
        Authorizations res;
        try {
            res = authorizationsService.getAuthorizationsByLearningUnitId(id);
        } catch (Exception e) {
            return new RESTResponse<Authorizations>(RESTResponse.DBFAIL, "Inconsistencia en la base de datos.", null);
        }
        return new RESTResponse<Authorizations>(RESTResponse.OK, "", res);
